...
The Java Virtual Machine dynamically loads, links and initializes classes and interfaces. Loading is the process of finding the binary representation of a class or interface type with a particular name and _creating_ a class or interface from that binary representation. Linking is the process of taking a class or interface and combining it into the run-time state of the Java Virtual Machine so that it can be executed. Initialization of a class or interface consists of executing the class or interface initialization method \([§2.9](chapter-2.-the-structure-of-the-java-virtual-machine.md#jvms-2.9)\).

In this chapter, [§5.1](chapter-5.-loading-linking-and-initializing.md#jvms-5.1) describes how the Java Virtual Machine derives symbolic references from the binary representation of a class or interface. [§5.2](chapter-5.-loading-linking-and-initializing.md#jvms-5.2) explains how the processes of loading, linking, and initialization are first initiated by the Java Virtual Machine. [§5.3](chapter-5.-loading-linking-and-initializing.md#jvms-5.3) specifies how binary representations of classes and interfaces are loaded by class loaders and how classes and interfaces are created. Linking is described in [§5.4](chapter-5.-loading-linking-and-initializing.md#jvms-5.4). [§5.5](chapter-5.-loading-linking-and-initializing.md#jvms-5.5) details how classes and interfaces are initialized. [§5.6](chapter-5.-loading-linking-and-initializing.md#jvms-5.6) introduces the notion of binding native methods. Finally, [§5.7](chapter-5.-loading-linking-and-initializing.md#jvms-5.7) describes when a Java Virtual Machine exits.
...
The Java Virtual Machine starts up by creating an initial class, which is specified in an implementation-dependent manner, using the bootstrap class loader \([§5.3.1](chapter-5.-loading-linking-and-initializing.md#jvms-5.3.1)\). The Java Virtual Machine then links the initial class, initializes it, and invokes the `public` class method `void main(String[])`. The invocation of this method drives all further execution. Execution of the Java Virtual Machine instructions constituting the `main` method may cause linking \(and consequently creation\) of additional classes and interfaces, as well as invocation of additional methods.
...
* If `N` denotes a nonarray class or an interface, one of the two following methods is used to load and thereby create C:
  * If D was defined by the bootstrap class loader, then the bootstrap class loader initiates loading of C \([§5.3.1](chapter-5.-loading-linking-and-initializing.md#jvms-5.3.1)\).
  * If D was defined by a user-defined class loader, then that same user-defined class loader initiates loading of C \([§5.3.2](chapter-5.-loading-linking-and-initializing.md#jvms-5.3.2)\).
* Otherwise `N` denotes an array class. An array class is created directly by the Java Virtual Machine \([§5.3.3](chapter-5.-loading-linking-and-initializing.md#jvms-5.3.3)\), not by a class loader. However, the defining class loader of D is used in the process of creating array class C.
...
If the Java Virtual Machine ever attempts to load a class C during verification \([§5.4.1](chapter-5.-loading-linking-and-initializing.md#jvms-5.4.1)\) or resolution \([§5.4.3](chapter-5.-loading-linking-and-initializing.md#jvms-5.4.3)\) \(but not initialization \([§5.5](chapter-5.-loading-linking-and-initializing.md#jvms-5.5)\)\), and the class loader that is used to initiate loading of C throws an instance of `ClassNotFoundException`, then the Java Virtual Machine must throw an instance of `NoClassDefFoundError` whose cause is the instance of `ClassNotFoundException`.

\(A subtlety here is that recursive class loading to load superclasses is performed as part of resolution \([§5.3.5](chapter-5.-loading-linking-and-initializing.md#jvms-5.3.5), step 3\). Therefore, a `ClassNotFoundException` that results from a class loader failing to load a superclass must be wrapped in a `NoClassDefFoundError`.\)
...
Then the Java Virtual Machine attempts to derive a class denoted by `N` using the bootstrap class loader from the purported representation using the algorithm found in [§5.3.5](chapter-5.-loading-linking-and-initializing.md#jvms-5.3.5). That class is C.
...
1. If the component type is a `reference` type, the algorithm of this section \([§5.3](chapter-5.-loading-linking-and-initializing.md#jvms-5.3)\) is applied recursively using class loader `L` in order to load and thereby create the component type of C.
2. The Java Virtual Machine creates a new array class with the indicated component type and number of dimensions.
   ...
   In any case, the Java Virtual Machine then records that `L` is an initiating loader for C \([§5.3.4](chapter-5.-loading-linking-and-initializing.md#jvms-5.3.4)\).
...
To ensure this, the Java Virtual Machine imposes _loading constraints_ of the form `NL1` = `NL2` during preparation \([§5.4.2](chapter-5.-loading-linking-and-initializing.md#jvms-5.4.2)\) and resolution \([§5.4.3](chapter-5.-loading-linking-and-initializing.md#jvms-5.4.3)\). To enforce these constraints, the Java Virtual Machine will, at certain prescribed times \(see [§5.3.1](chapter-5.-loading-linking-and-initializing.md#jvms-5.3.1), [§5.3.2](chapter-5.-loading-linking-and-initializing.md#jvms-5.3.2), [§5.3.3](chapter-5.-loading-linking-and-initializing.md#jvms-5.3.3), and [§5.3.5](chapter-5.-loading-linking-and-initializing.md#jvms-5.3.5)\), record that a particular loader is an initiating loader of a particular class. After recording that a loader is an initiating loader of a class, the Java Virtual Machine must immediately check to see if any loading constraints are violated. If so, the record is retracted, the Java Virtual Machine throws a `LinkageError`, and the loading operation that caused the recording to take place fails.

Similarly, after imposing a loading constraint \(see [§5.4.2](chapter-5.-loading-linking-and-initializing.md#jvms-5.4.2), [§5.4.3.2](chapter-5.-loading-linking-and-initializing.md#jvms-5.4.3.2), [§5.4.3.3](chapter-5.-loading-linking-and-initializing.md#jvms-5.4.3.3), and [§5.4.3.4](chapter-5.-loading-linking-and-initializing.md#jvms-5.4.3.4)\), the Java Virtual Machine must immediately check to see if any loading constraints are violated. If so, the newly imposed loading constraint is retracted, the Java Virtual Machine throws a `LinkageError`, and the operation that caused the constraint to be imposed \(either resolution or preparation, as the case may be\) fails.
...
4. If C has any direct superinterfaces, the symbolic references from C to its direct superinterfaces are resolved using the algorithm of [§5.4.3.1](chapter-5.-loading-linking-and-initializing.md#jvms-5.4.3.1).
...
5. The Java Virtual Machine marks C as having `L` as its defining class loader and records that `L` is an initiating loader of C \([§5.3.4](chapter-5.-loading-linking-and-initializing.md#jvms-5.3.4)\).
...
_Preparation_ involves creating the static fields for a class or interface and initializing such fields to their default values \([§2.3](chapter-2.-the-structure-of-the-java-virtual-machine.md#jvms-2.3), [§2.4](chapter-2.-the-structure-of-the-java-virtual-machine.md#jvms-2.4)\). This does not require the execution of any Java Virtual Machine code; explicit initializers for static fields are executed as part of initialization \([§5.5](chapter-5.-loading-linking-and-initializing.md#jvms-5.5)\), not preparation.

During preparation of a class or interface C, the Java Virtual Machine also imposes loading constraints \([§5.3.4](chapter-5.-loading-linking-and-initializing.md#jvms-5.3.4)\). Let `L1` be the defining loader of C. For each method `m` declared in C that overrides \([§5.4.5](chapter-5.-loading-linking-and-initializing.md#jvms-5.4.5)\) a method declared in a superclass or superinterface `<`D, `L2>`, the Java Virtual Machine imposes the following loading constraints:

Given that the return type of `m` is Tr, and that the formal parameter types of `m` are Tf1, ..., Tfn, then:

If Tr not an array type, let T0 be Tr; otherwise, let T0 be the element type \([§2.4](chapter-2.-the-structure-of-the-java-virtual-machine.md#jvms-2.4)\) of Tr.

For _i_ = 1 to _n_: If Tfi is not an array type, let Ti be Tfi; otherwise, let Ti be the element type \([§2.4](chapter-2.-the-structure-of-the-java-virtual-machine.md#jvms-2.4)\) of Tfi.
...
The following sections describe the process of resolving a symbolic reference in the run-time constant pool \([§5.1](chapter-5.-loading-linking-and-initializing.md#jvms-5.1)\) of a class or interface D. Details of resolution differ with the kind of symbolic reference to be resolved.
...
1. The defining class loader of D is used to create a class or interface denoted by `N`. This class or interface is C. The details of the process are given in [§5.3](chapter-5.-loading-linking-and-initializing.md#jvms-5.3).
...
2. If C is an array class and its element type is a `reference` type, then a symbolic reference to the class or interface representing the element type is resolved by invoking the algorithm in [§5.4.3.1](chapter-5.-loading-linking-and-initializing.md#jvms-5.4.3.1) recursively.
3. Finally, access permissions to C are checked.
   * If C is not accessible \([§5.4.4](chapter-5.-loading-linking-and-initializing.md#jvms-5.4.4)\) to D, class or interface resolution throws an `IllegalAccessError`.
...
* Otherwise, if field lookup succeeds but the referenced field is not accessible \([§5.4.4](chapter-5.-loading-linking-and-initializing.md#jvms-5.4.4)\) to D, field resolution throws an `IllegalAccessError`.
* Otherwise, let `<`E, `L1>` be the class or interface in which the referenced field is actually declared and let `L2` be the defining loader of D.
  ...
  Given that the type of the referenced field is Tf, let T be Tf if Tf is not an array type, and let T be the element type \([§2.4](chapter-2.-the-structure-of-the-java-virtual-machine.md#jvms-2.4)\) of Tf otherwise.

  The Java Virtual Machine must impose the loading constraint that T`L1` = T`L2` \([§5.3.4](chapter-5.-loading-linking-and-initializing.md#jvms-5.3.4)\).

#### 5.4.3.3. Method Resolution

To resolve an unresolved symbolic reference from D to a method in a class C, the symbolic reference to C given by the method reference is first resolved \([§5.4.3.1](chapter-5.-loading-linking-and-initializing.md#jvms-5.4.3.1)\). Therefore, any exception that can be thrown as a result of failure of resolution of a class reference can be thrown as a result of failure of method resolution. If the reference to C can be successfully resolved, exceptions relating to the resolution of the method reference itself can be thrown.

When resolving a method reference:

1.  If C is an interface, method resolution throws an `IncompatibleClassChangeError`.
2.  Otherwise, method resolution attempts to locate the referenced method in C and its superclasses:
   *  If C declares exactly one method with the name specified by the method reference, and the declaration is a signature polymorphic method \([§2.9](chapter-2.-the-structure-of-the-java-virtual-machine.md#jvms-2.9)\), then method lookup succeeds. All the class names mentioned in the descriptor are resolved \([§5.4.3.1](chapter-5.-loading-linking-and-initializing.md#jvms-5.4.3.1)\).
...
*  Otherwise, if method lookup succeeds and the referenced method is not accessible \([§5.4.4](chapter-5.-loading-linking-and-initializing.md#jvms-5.4.4)\) to D, method resolution throws an `IllegalAccessError`.
*  Otherwise, let `<`E, `L1>` be the class or interface in which the referenced method `m` is actually declared, and let `L2` be the defining loader of D.
   ...
   If Tr is not an array type, let T0 be Tr; otherwise, let T0 be the element type \([§2.4](chapter-2.-the-structure-of-the-java-virtual-machine.md#jvms-2.4)\) of Tr.

   For _i_ = 1 to _n_: If Tfi is not an array type, let Ti be Tfi; otherwise, let Ti be the element type \([§2.4](chapter-2.-the-structure-of-the-java-virtual-machine.md#jvms-2.4)\) of Tfi.

   The Java Virtual Machine must impose the loading constraints Ti`L1` = Ti`L2` for _i_ = 0 to _n_ \([§5.3.4](chapter-5.-loading-linking-and-initializing.md#jvms-5.3.4)\).
...
4.  Otherwise, if the maximally-specific superinterface methods \([§5.4.3.3](chapter-5.-loading-linking-and-initializing.md#jvms-5.4.3.3)\) of C for the name and descriptor specified by the method reference include exactly one method that does not have its `ACC_ABSTRACT` flag set, then this method is chosen and method lookup succeeds.
...
*  If method lookup succeeds and the referenced method is not accessible \([§5.4.4](chapter-5.-loading-linking-and-initializing.md#jvms-5.4.4)\) to D, interface method resolution throws an `IllegalAccessError`.
...
Let `MH` be the symbolic reference to a method handle \([§5.1](chapter-5.-loading-linking-and-initializing.md#jvms-5.1)\) being resolved. Then:
...
*  Third, a reference to an instance of `java.lang.invoke.MethodType` is obtained as if by resolution of an unresolved symbolic reference to a method type that contains the method descriptor specified in [Table 5.4.3.5-B](chapter-5.-loading-linking-and-initializing.md#jvms-5.4.3.5-260) for the kind of `MH`.

  It is as if the symbolic reference to a method handle contains a symbolic reference to the method type that the resolved method handle will eventually have. The detailed structure of the method type is obtained by inspecting [Table 5.4.3.5-B](chapter-5.-loading-linking-and-initializing.md#jvms-5.4.3.5-260).
...
During resolution of the symbolic reference to the method handle in the call site specifier, or resolution of the symbolic reference to the method type for the method descriptor in the call site specifier, or resolution of a symbolic reference to any static argument, any of the exceptions pertaining to method type or method handle resolution may be thrown \([§5.4.3.5](chapter-5.-loading-linking-and-initializing.md#jvms-5.4.3.5)\).

A class or interface C is _accessible_ to a class or interface D if and only if either of the following is true:

*  C is `public`.
*  C and D are members of the same run-time package \([§5.3](chapter-5.-loading-linking-and-initializing.md#jvms-5.3)\).
...
*  One of the following is true:
 ...
 _Initialization_ of a class or interface consists of executing its class or interface initialization method \([§2.9](chapter-2.-the-structure-of-the-java-virtual-machine.md#jvms-2.9)\).

 A class or interface C may be initialized only as a result of:

*  The execution of any one of the Java Virtual Machine instructions _new_, _getstatic_, _putstatic_, or _invokestatic_ that references C \([§_new_](), [§_getstatic_](), [§_putstatic_](), [§_invokestatic_]()\). These instructions reference a class or interface directly or indirectly through either a field reference or a method reference.
...
*  The first invocation of a `java.lang.invoke.MethodHandle` instance which was the result of method handle resolution \([§5.4.3.5](chapter-5.-loading-linking-and-initializing.md#jvms-5.4.3.5)\) for a method handle of kind 2 \(`REF_getStatic`\), 4 \(`REF_putStatic`\), 6 \(`REF_invokeStatic`\), or 8 \(`REF_newInvokeSpecial`\).

   This implies that the class of a bootstrap method is initialized when the bootstrap method is invoked for an _invokedynamic_ instruction \([§_invokedynamic_]()\), as part of the continuing resolution of the call site specifier.

*  Invocation of certain reflective methods in the class library \([§2.12](chapter-2.-the-structure-of-the-java-virtual-machine.md#jvms-2.12)\), for example, in class `Class` or in package `java.lang.reflect`.
...
*  If C is a class, its designation as the initial class at Java Virtual Machine startup \([§5.2](chapter-5.-loading-linking-and-initializing.md#jvms-5.2)\).
